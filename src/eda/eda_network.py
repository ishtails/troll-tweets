...
def extract_hashtag_network(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {"nodes": nodes, "edges": edges, "has_network": len(edges) > 0}
...
def analyze_networks(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with network analysis results in JSON-compatible format
    """
    result = {}
    ...
    result["summary"] = {
        "has_networks": hashtag_network["has_network"]
        or mention_network["has_network"],
        "hashtag_network_size": len(hashtag_network["nodes"]),
        "hashtag_network_density": (
            len(hashtag_network["edges"])
            / (len(hashtag_network["nodes"]) * (len(hashtag_network["nodes"]) - 1) / 2)
            if len(hashtag_network["nodes"]) > 1
            else 0
        ),
        "mention_network_size": len(mention_network["nodes"]),
        "mention_network_density": (
            len(mention_network["edges"])
            / (len(mention_network["nodes"]) * (len(mention_network["nodes"]) - 1) / 2)
            if len(mention_network["nodes"]) > 1
            else 0
        ),
    }

    # Ensure all numeric values are native Python types, not numpy types
    for network_type in ["hashtag_network", "mention_network"]:
        if network_type in result:
            # Convert node weights to native Python types
            for node in result[network_type]["nodes"]:
                if "weight" in node:
                    node["weight"] = int(node["weight"])

            # Convert edge weights to native Python types
            for edge in result[network_type]["edges"]:
                if "weight" in edge:
